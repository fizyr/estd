...
namespace convert_detail {
	template<typename F, typename T, typename Tag, typename = void> struct has_impossible : std::false_type{};
	template<typename F, typename T, typename Tag> struct has_impossible<F, T, Tag, std::void_t<decltype(&::estd::conversion<F, T, Tag>::impossible)>> : std::true_type {};
	...
	template<typename F, typename T, typename Tag,
		bool = has_impossible<F, T, Tag>(),
		bool = has_perform<F, T, Tag>
	> struct can_convert : std::false_type{};

	template<typename F, typename T, typename Tag>
	struct can_convert<F, T, Tag, true, true> : std::integral_constant<bool, conversion<F, T, Tag>::impossible == false> {};
